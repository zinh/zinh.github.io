...
The story becomes interesting when `blocking: false`(this is the default value of `blocking`). It will make this fiber to become a non-blocking fiber.
...
Now, let implement a simple `kernel_sleep` scheduler

### kernel_sleep

`kernel_sleep` hook will be called for event such as `Kernel.sleep`. Our scheduler will need to support `kernel_sleep` and `close`
...
To use our `SimpleFiber`, we will need to call `Fiber.set_scheduler`
...
class SimpleScheduler
  def run
    while @readable.any? or @writable.any? or @waiting.any?
      ...
      # we remove it from our @readable/@writable and resume the corresponding fiber
      readable&.each do |io|
...
which implies that somewhere in http library, `block/unblock` hook is called.
...
def run
  ...
  # @readable: for fiber that are waiting to read
  # @writable: for fiber that are waiting to write
  # @waiting: for fiber that are sleeping
  # @blocking: for fiber that are blocked
  # @ready: for fiber that are unblocked
...
To us user, we probaly don't deal directly with fiber but through other wrapper such as [Async](https://github.com/socketry/async).
...
Lastly, there're some improvements that I'll leave as an exercise to you
